...
class OutletMapper {
	...
	static function insert (&$obj) {
		...
		foreach ($insert_props as $key=>$p) {
			...
			//hack to fix weird behavior here.
			$val=self::toSqlValue($properties[$p], self::getProp($obj, $p));
			$val=(is_null($val)) ? 'Unknown' : $val;
	...
	static function toArray ($entity) {
		...
		foreach (Outlet::getInstance()->getConfig()->getEntity($class)->getProperties() as $key=>$p) {
			$var=self::getProp($entity, $key);
			$arr[$key] = self::toSqlValue($p, $var);
		}
		return $arr;
	}
