...
class ActionController2
{
	...
	public function executeCreatethelink( )
        {
                ...
                if($p) header('Location: '.SBASEHREF.'sindex.php?p=index'); 
	}
    ...
    {       
		...
		header('Location: '.SBASEHREF.'admin.php?p=comments');
	}
	...
	{       
		...
		$mongo->test->wp_term->update(array('_id'=>new MongoId($catid) ), array('$set'=>$updates));
		header('Location: '.SBASEHREF.'sindex.php?p=index');
	}
	...
	public function executeCreatep( )
	{
		...
		if($p) header('Location: '.SBASEHREF.'admin.php?p=edit&pid=' .$p->getId() );	
	}
    ...
    {   
        ...
        if($retval['ok'])
        {
            if($post_status=='publish')
                header(sprintf('Location: '.SBASEHREF.'sindex.php?p=%s&pid=%s','postdetail', $pid));
            else
                header(sprintf('Location: '.SBASEHREF.'admin.php?p=%s','index')); //&pid=%s',$pid))
        }
	...
	public function executeDeltag()
    {
        ...
        if($ok > 0)
                header('Location: '.SBASEHREF.'admin.php?p=tags'); // admin.php?p=links
	...
	public function executeDelcat()
    {
        ...
        if($ok > 0)
                header('Location: '.SBASEHREF.'admin.php?p=categories'); // admin.php?p=links
	...
	{ 
		...
		if($ok > 0)
	    	header('Location: '.SBASEHREF.'admin.php?p=links'); // admin.php?p=links
	...
	public function executeDelcomm()
    {
        ...
        if($ok > 0)
                header('Location: '.SBASEHREF.'admin.php?p=comments'); // admin.php?p=links
	...
	public function executeDelp() 
	{
		...
		if($ok > 0) 
			header('Location: '.SBASEHREF.'admin.php?p=index&s=1');
	}
